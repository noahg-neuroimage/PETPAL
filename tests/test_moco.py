...
def apply_random_xfms(static_img: ants.ANTsImage, center: list, nframes: int=4):
    ...
    for _i in range(nframes):
        ...
        ants_xfm_rot = ants.contrib.affine3d.Rotate3D(rotation=tuple(rotation_pars)).transform().parameters
        ants_xfm_tra = ants.contrib.affine3d.Translate3D(translation=tuple(translation_pars)).transform().parameters
...
def test_motion():
    ...
    Read in result. Check if it matches the applied transforms up to an absolute tolerance of 0.1
    mm or degree and a relative tolerance of 0.5 for all parameters."""

    static_img_path = 'FDG_static_blur.nii.gz' # need a static test image
    static_img = ants.image_read(static_img_path)
    center = [0,0,0] # insert center of the test image

    sim_img, by_frame = apply_random_xfms(static_img=static_img, center=center, nframes=9)
    ...
    np.testing.assert_allclose(xfm_applied,np.asarray(by_frame),rtol=0.5,atol=0.1)
